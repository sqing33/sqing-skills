#!/usr/bin/env python3
"""Collect Bilibili daily-news hotspots when the user explicitly requests Bilibili.
...
# Default UP list (used when no --up / --up-file / --video is given).
# Each entry can be a mid, space URL, or username; mid is preferred for stability.
DEFAULT_UPS: list[str] = [
    "285286947",          # 橘鸦Juya
    "3493082576193678",   # infinite灵感港
    "3706929260006322",   # 黑鸦Heya
]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not ups and not videos:
        # No UP / video explicitly given -> fall back to the default 3-UP list.
        # Pass --up with at least one entry to override the default.
        ups = list(DEFAULT_UPS)
        print(
            json.dumps(
                {
                    "_hint": "no --up / --video given; using DEFAULT_UPS list",
