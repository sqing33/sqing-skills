---
name: ai-daily-hotspot-xhs
description: 聚合开放 RSS、Atom 和 JSON API 中的最新 AI 热点，覆盖官方发布、行业新闻、模型、论文与开源项目。脚本只负责收集、规范化、去重，不过滤任何内容；所有选题、评分和优先级排序由 AI 分析师完成。再用 HTTP 优先、内嵌 `control-in-app-browser` skill 兜底的服务端流程解析跳转、定位一手来源、提取正文和图片线索（通过 `asset_fetcher` 子模块支持 curl 直链 / web_fetch grep / Browser 截图三种工作流），生成每日 7–10 条新闻简报式小红书或公众号内容；支持 Linux、Docker、CI 和定时任务，不依赖桌面浏览器。只有用户明确提供 B 站 UP、BV 号或要求 B 站时才启用可选 B 站采集。
---
...
**目标**:每篇报道配 1–5 张有信息价值的图,**必须**用 `![alt](images/xxx.png)` Markdown 语法**真正嵌入**最终 .md 文件 —— 用户在小红书/公众号场景下打开时才能渲染出图;**只写路径字符串 = 没做**。
...
只使用已核验来源中的事实。默认写成客观、信息密集的 AI 新闻简报；不要写第一人称体验、夸张排名、无来源价格、编造用户反馈或绝对化结论。

公众号、媒体早报、Google News、HN、B 站简介或评论只用于发现选题和定位链接，不能作为洗稿素材。正文不得出现“UP 主称”“视频里说”“其他早报提到”等表述。
...
完成核验后输出一个包含 7–10 条内容的 Markdown 文档；默认目标 8 条。
...
| # | 发布时间 | 热点 | 线索来源 | 事实来源 | 图片建议 | 可信度 | 风险 |
|---|---|---|---|---|---|---|---|
| 1 | <时间> | <热点短标题> | <来源名和候选 URL> | <一手 URL> | <页面/区块/素材> | 高/中/低 | <风险或“无”> |
...
**正文**：
<先概括，再写关键参数、能力、开放方式、适用场景和限制。>

#标签1 #标签2 #标签3

**图片**：

![Qwen3.8-Max 官方 banner](images/qwen38-banner.jpg)
- 来源：qwen.ai 官方发布页
- 区块：产品 banner，含版本号和视觉标识
- 用途：主图
- 备注：官方公开素材
...
- 总览与素材核验只用一张表，列名固定为 `#`、`发布时间`、`热点`、`线索来源`、`事实来源`、`图片建议`、`可信度`、`风险`。
- 每个已核验热点默认单独成篇；用户限制数量时只写指定数量。
- 默认日报只收录 7–10 条通过核验的热点；不足 7 条时允许少发，不得用噪声或未核实条目补齐。
- 标题控制在 20 字以内，保留主体、产品或模型名与核心动作；参数和日期放正文。
- 正文后直接写 `#标签`，不要添加“标签：”，不要使用 `#AI早报` 这类合集标签。
- 图片可多张，但每张都必须 `![alt](images/文件名)` 真正嵌入（不是只写路径字符串），下面再跟 `来源 / 区块 / 用途 / 备注` 四行元数据。
...
- [ ] 7-10 条热点全部写齐，标题 ≤ 20 字
- [ ] 总览表 8 列对齐：`# / 发布时间 / 热点 / 线索来源 / 事实来源 / 图片建议 / 可信度 / 风险`
...
- [ ] **🔴 每张配图的来源已在 4 行元数据"来源"字段标出优先级(1 官方 / 2 权威英文媒体 / 3 中文媒体转载)**:任何降级到 3 的图,必须同步在"备注"里写明降级原因(为什么 1+2 拿不到)。**默认期望所有图都走优先级 1,不允许"默认走腾讯 CDN"**。
